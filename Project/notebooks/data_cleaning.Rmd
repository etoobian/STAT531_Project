...
|                                       |
|:-------------------------------------:|
| **Project/notebooks/data_cleaning.R** |
...
# RStudio: Ensure Knit Directory is set to Project Directory
# (Set this in Knit options, not in code.)

# Load shared data I/O utilities and raw data
source("Project/scripts/data_io.R")
ad_data <- load_ad_data(preview = FALSE)
...
```{r}
discov_data <- ad_data
discov_data <- mutate(.data=discov_data, PRICE=as.numeric(PRICE)) # NAs introduced by coercion.
discov_idx <- which(is.na(discov_data$PRICE)) # Where the NA exists.
discov_idx
ad_data$PRICE[discov_idx] # Where NA values are introduced.
```
...
```{r}
discov_data <- ad_data
discov_data$TIMESTAMP <- sub("\\s[^\\s]*$", "", discov_data$TIMESTAMP) # Remove the last portion of a string separated by a whitespace.
unique(discov_data$TIMESTAMP) 
```
...
**Discovery Method:** This was discovered by checking for values outside of Oregon's border (-124'38 to -116.28).

```{r}
discov_data <- ad_data
discov_data <- filter(.data=discov_data, DEVICE_GEO_LONG < -125 | DEVICE_GEO_LONG > -116)
discov_data$DEVICE_GEO_LONG
count(discov_data)
```
...
lat_of_has_over_long_val <- filter(.data=temp_ad_data, DEVICE_GEO_LONG < -130)$DEVICE_GEO_LAT # Lat values where DEVICE_GEO_LONG < -130
ungroup(filter(.data=summarize(.data=group_by(.data=filter(.data=temp_ad_data, DEVICE_GEO_LAT %in% lat_of_has_over_long_val), DEVICE_GEO_LAT, DEVICE_GEO_ZIP), n=n_distinct(DEVICE_GEO_LONG)), n>1)) # Initial print out of those grouped together by Lat and Zip having more than 2 longitudinal values.
temp_ad_data <- mutate(.data=temp_ad_data, DEVICE_GEO_LONG=if_else(DEVICE_GEO_LONG < -130, DEVICE_GEO_LONG + 10, DEVICE_GEO_LONG)) # Add 10 to those values.
temp_ad_data <- mutate(.data=temp_ad_data, DEVICE_GEO_LAT=round(DEVICE_GEO_LAT, 10), DEVICE_GEO_LONG=round(DEVICE_GEO_LONG, 10)) # Round to account for floating-point precision errors introduced.
ungroup(filter(.data=summarize(.data=group_by(.data=filter(.data=temp_ad_data, DEVICE_GEO_LAT %in% lat_of_has_over_long_val), DEVICE_GEO_LAT, DEVICE_GEO_ZIP), n=n_distinct(DEVICE_GEO_LONG)), n>1)) # Final print out of those grouped together... (contains nothing).
rm(lat_of_has_over_long_val, temp_ad_data)
```
...
```{r}
filter(mutate(.data=ad_data, PRICE=as.numeric(PRICE)), PRICE < 0)$PRICE
```
...
```{r}
set.seed(123)
hist(sample(as.numeric(ad_data$PRICE), size=1000, replace=FALSE), breaks=150, xlim=c(0, 3)) # Histogram of positive values.
hist(as.numeric(filter(.data=ad_data, as.numeric(PRICE) < 0, as.numeric(PRICE) > -10)$PRICE), breaks=50, xlim=c(0, -3)) # Histogram of the negative values.
```

\- PRICE <chr>: There exist prices that are -999.\
**Discovery Method:** This was discovered by checking the histogram of negative values.

```{r}
filter(mutate(.data=ad_data, PRICE=as.numeric(PRICE)), PRICE < -10)$PRICE
```
...
```{r}
filter(mutate(.data=ad_data, DEVICE_GEO_ZIP=as.integer(DEVICE_GEO_ZIP)), DEVICE_GEO_ZIP < 0)$DEVICE_GEO_ZIP
```
...
```{r}
discov_data <- ad_data
discov_data$RESPONSE_TIME <- sub("^RESPONSE_TIME: ", "", discov_data$RESPONSE_TIME) # Remove prefix "RESPONSE_TIME: " from RESPONSE_TIME.
discov_data <- mutate(.data=discov_data, RESPONSE_TIME=as.integer(RESPONSE_TIME)) # NAs introduced by coercion.
discov_idx <- which(is.na(discov_data$RESPONSE_TIME)) # Where the NAs exist.
discov_idx
ad_data$RESPONSE_TIME[discov_idx] # Where NA values are introduced.
```
...
-   Noted that at least some AUCTION_ID values are encompassed purely by one bidder in PUBLISHER_ID (one bidder bidding multiple times) (Given by multiple bidders bidding through a publisher).

-   For further exploration, we can potentially re-order these instances in chronological order by RESPONSE_TIME

-   For some of these discoveries, some columns were converted to numeric data types (int, numeric) then had pairs ran on them.

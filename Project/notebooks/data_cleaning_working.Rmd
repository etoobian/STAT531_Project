...
```{r overview-dims, message=FALSE, results='hold'}
# Dimensions
n_rows <- nrow(ad_raw)
n_cols <- ncol(ad_raw)
...
### Summary
  ...
  - `DEVICE_GEO_REGION` is stored as a `character` field and has no missing values.
  - All Oregon variants (`"OR"`, `"Or"`, `"oregon"`, `"xor"`) were standardized to a single clean code, `"OR"`.
  - No rows were removed based during this cleaning process.  
...
- `REQUESTED_SIZES`  
  - **Type (dictionary):** `categorical`  (string or array of sizes)
  - **Meaning:** All ad slot sizes that were requested for the impression (e.g., `"300x250, 320x50"`).  
  - **Expectation:** A non-missing, delimited list of size codes in `width x height` format.  
  - **Actual Type:** `character`
  - **Missing:** None

- `SIZE`  
  - **Type (dictionary):** `categorical`  
  - **Meaning:** The size associated with this bid line (typically one of the `REQUESTED_SIZES`).  
  - **Expectation:** A single size in `width x height` format (e.g., `"300x250"`), matching one of the requested sizes and representing a valid display slot.  
  - **Actual type:** `character`  
  - **Missing:** None  

### Exploration

We first check the **storage format** of `REQUESTED_SIZES` and then look for structurally unusual `SIZE` values.

```{r ad-sizes-explore, results='hold'}
# Sample REQUESTED_SIZES to understand its raw format
sample(ad_clean$REQUESTED_SIZES, size = 10)

# Structural sanity checks for REQUESTED_SIZES:
#   - looks like a JSON array (starts with "[" and ends with "]")
#   - contains at least one "x" character (size-like tokens)
requested_json_like <- str_starts(ad_clean$REQUESTED_SIZES, "\\[") &
  str_ends(ad_clean$REQUESTED_SIZES, "\\]")

requested_contains_x <- str_detect(ad_clean$REQUESTED_SIZES, "x")

n_requested_not_json <-
  sum(!requested_json_like | is.na(requested_json_like))
n_requested_no_x     <-
  sum(!requested_contains_x | is.na(requested_contains_x))

cat("REQUESTED_SIZES not bracketed like JSON array: ",
    n_requested_not_json,
    "\n")
cat("REQUESTED_SIZES with no 'x' character:         ", n_requested_no_x, "\n")

# Count distinct SIZE values and show most common ones
size_summary <- ad_clean %>%
  count(SIZE, sort = TRUE)

head(size_summary, 15)

# Count clearly non-physical sizes we know about ("0x0", "1x1")
size_invalid_counts <- ad_clean %>%
  filter(SIZE %in% c("0x0", "1x1")) %>%
  count(SIZE, sort = TRUE)

size_invalid_counts

# Specifically check how many of these "invalid" sizes correspond to winning bids
invalid_size_won <- ad_clean %>%
  filter(SIZE %in% c("0x0", "1x1"),
         BID_WON %in% c("TRUE", "true")) %>%
  select(AUCTION_ID, SIZE, BID_WON)

n_invalid_size_won <- nrow(invalid_size_won)

cat(
  "Rows with SIZE in {\"0x0\",\"1x1\"} and BID_WON == TRUE/\"true\": ",
  n_invalid_size_won,
  "\n"
)
```
...
**Observations**
  ...
  - `REQUESTED_SIZES` is consistently stored as a JSON-encoded array (all rows begin with `[` and end with `]`, and contain size-like tokens such as `"320x50"`). 
      - We do not need to modify this field for cleaning.
  - `SIZE` mostly contains standard ad dimensions (e.g., `"320x50"`, `"300x250"`), but includes a small number of values such as `"0x0"` (901 rows) and `"1x1"` (52 rows).
  - These `"0x0"` / `"1x1"` values also appear in **winning** rows (327 cases), but they do not correspond to a physical display size. 
      - Without additional system documentation, their operational meaning is unclear.

**Decisions**

  - Preserve `REQUESTED_SIZES` as stored; downstream analysis can parse the JSON if needed.
  - Because `"0x0"` and `"1x1"` cannot be interpreted as physical ad sizes and their meaning is unknown, the cleaned size field (`SIZE_clean`) will treat these values as **missing** (`NA`).  
      - The raw `SIZE` column remains unchanged.  
  - All other `SIZE` values will be carried forward as-is.

### Cleaning

No transformations are required for either `REQUESTED_SIZES` or `SIZE`.

Although `SIZE` includes values such as `"0x0"` and `"1x1"`, we do not have domain documentation confirming that these codes are invalid or erroneous. 
They may represent upstream placeholders or system-level codes, and therefore we preserve them exactly as recorded.

For schema consistency, we create pass-through cleaned columns:

```{r ad-sizes-clean, results='hold'}
ad_clean <- ad_clean %>%
  mutate(
...
Because there are **no persistent changes**, **no log entry is added** for this variable group.

### Summary

  - `REQUESTED_SIZES` is consistently stored as a JSON-encoded array of size strings.
      - No cleaning was required, and the field is preserved as `REQUESTED_SIZES_clean`.
  - `SIZE` contains mostly standard dimensions, with a small number of unusual codes (e.g., `"0x0"`, `"1x1"`).  
      - Because their meaning is unknown and not documented as erroneous, these values are preserved exactly and exposed as `SIZE_clean` without modification.
  - No rows were removed and no values were altered in the ad-size fields.
...
- `PRICE`  
  - **Type (dictionary):** `numeric` (CPM bid in USD)  
  - **Meaning:** Bid price in cost per thousand impressions (CPM).  
  - **Expectation:** Non-negative real number; typically small (e.g., \$0–\$50).  
  - **Actual Type:** `character`  
  - **Missing:** None

### Exploration

`PRICE` has already been confirmed to be stored as `character`, we now inspect the characters used in `PRICE`.

```{r price-characters}
# Investigate all unique characters present in PRICE strings
chars <- strsplit(ad_clean$PRICE, split = "")
...
We investigate each of these further.

```{r price-explore, results='hold'}
# Rows with the letter 'O'
price_O_rows <- ad_clean %>% filter(str_detect(PRICE, "O"))
n_price_O    <- nrow(price_O_rows)
head(price_O_rows$PRICE)

# Rows containing 'e' (scientific notation)
price_e_rows <- ad_clean %>% filter(str_detect(PRICE, "e"))
n_price_e    <- nrow(price_e_rows)
head(price_e_rows$PRICE)

# Rows with a negative sign '-'
price_neg_rows <- ad_clean %>% filter(str_detect(PRICE, "-"))
n_price_neg_strings <- nrow(price_neg_rows)
head(price_neg_rows$PRICE)

# Frequency table of negative PRICE strings
table(price_neg_rows$PRICE)
```

**Observations**

  - `PRICE` is stored as `character`, but the data dictionary indicates it should be numeric.  
  - The character set includes digits, decimal points, minus signs, `"e"` (scientific notation), and a single `"O"` at the start of one value (a likely typo for `"0"`).
  - Rows containing:
      - A leading `"O"` in `PRICE` appear exactly once (a clear typographical error for a numeric value).  
      - `"e"` correspond to 3 rows in scientific notation.  
      - A minus sign (`"-"`) appear in 115 rows, consisting of:
          - 12 values equal to `"-999"`, which behave as sentinel codes rather than real prices.  
          - 3 negative scientific-notation values already covered above.  
          - 100 negative values that are plausible in magnitude but have the wrong sign (e.g., `"-0.34"`).

**Decisions**

  - Convert `PRICE` to a numeric field in a staged way:
      - Correct the single `"O"` $\to$ `"0"` typo before parsing.
      - Parse numeric values (including scientific notation) into a helper numeric column.
      - Treat `-999` as a sentinel code and convert it to `NA`.
  - After converting to numeric, examine the **distribution of negative prices** compared to positive prices:
      - If negative values look like sign errors (same magnitude as plausible bids, including some with `BID_WON == TRUE`), we will flip their sign in the final cleaned field.
      - Otherwise, we will leave them as-is or set them to `NA`.

### Cleaning 

We first correct obvious encoding issues, convert to `numeric`, and handle sentinel values.  
We use a helper numeric column (`PRICE_num`) for intermediate steps.

```{r price-clean-step1, results='hold'}
# Fix leading 'O' typo and parse to numeric (handles scientific notation)
ad_clean <- ad_clean %>%
  mutate(PRICE_str = str_replace(PRICE, "^O", "0"),
         PRICE_num = suppressWarnings(as.numeric(PRICE_str)))

# Count anomalies before adjustment
n_price_O        <-
  sum(str_detect(ad_clean$PRICE, "^O"), na.rm = TRUE)
n_price_sentinel <- sum(ad_clean$PRICE_num == -999, na.rm = TRUE)
n_price_neg_vals <-
  sum(ad_clean$PRICE_num < 0 &
        ad_clean$PRICE_num != -999, na.rm = TRUE)

cat("Rows with 'O' to '0' typo fixed: ", n_price_O, "\n")
cat("Rows with sentinel -999:         ", n_price_sentinel, "\n")
cat("Rows with negative values:       ", n_price_neg_vals, "\n")

# Treat -999 as sentinel and set to NA in the helper numeric column
ad_clean <- ad_clean %>%
  mutate(PRICE_num = if_else(PRICE_num == -999, NA_real_, PRICE_num))
```

We now examine the distribution of `numeric` prices (focusing on negative values) to justify how to handle them.

```{r price-diagnostic, results='hold'}
neg_prices <- ad_clean %>%
...
hist(
  sample(
    na.omit(ad_clean$PRICE_num),
    size    = min(1000, sum(!is.na(ad_clean$PRICE_num))),
    replace = FALSE
  ),
  main  = "Histogram of overall Prices",
  xlab  = "PRICE_num",
  breaks = 150,
  xlim = c(0, 3)
)
...
```
  ...
  - The magnitudes of negative prices are in the same general range as the positive bids.  
  - This supports interpreting them as sign errors rather than truly negative CPM bids.

We now define `PRICE_clean`:

  - Sentinel and malformed values remain `NA`.  
  - Negative numeric prices are flipped to positive (sign error correction).  
  - All other values are kept as-is.  
  - Helper columns are dropped.

```{r price-clean-step2, results='hold'}
ad_clean <- ad_clean %>%
  mutate(
    PRICE_clean = case_when(
      is.na(PRICE_num) ~ NA_real_,   # malformed or sentinel
      PRICE_num < 0    ~ -PRICE_num, # interpret negatives as sign errors
      TRUE             ~ PRICE_num   # all other values unchanged
    )
  ) %>%
  select(-PRICE_str, -PRICE_num)

# Total rows where PRICE was altered in any way
n_price_affected <- n_price_O + n_price_sentinel + n_price_neg_vals

cat("Rows with PRICE typo/sentinel/negative sign adjusted: ",
    n_price_affected, "\n")
```

Log entry for pricing variable:

```{r price-log, results='hold'}
log_change(
  step_id = "PR1",
  variable = "PRICE",
  issue = "PRICE stored as character with a leading 'O' typo, sentinel code -999, and 100 negative values of plausible magnitude.",
  decision = "Created numeric PRICE_clean; corrected leading 'O' to '0', parsed scientific notation, converted -999 to NA, and negated remaining negative prices to treat them as sign errors.",
  rows_affected = n_price_affected
)
```

### Summary

  - `PRICE` was recorded as `character` and contained a mix of plain decimals:
      - Scientific notation
      - One obvious `"O"` $\to$ `"0"` typo
      - Sentinel `-999` values
      - Negative prices with plausible magnitudes 
  - We created a cleaned numeric field `PRICE_clean` that:
      - Corrects the single `"O"` typo before parsing.  
      - Parses scientific notation correctly.  
      - Converts sentinel `-999` values to `NA`.  
      - Treats remaining negative prices as sign errors and flips them to positive, based on their distribution and the presence of winning bids at those magnitudes.  
  - No rows were dropped; invalid or unrecoverable price strings are represented as `NA` in `PRICE_clean`, preserving completeness while improving validity for downstream analysis.
...
### Description
  ...
  - `RESPONSE_TIME`  
    - **Type (dictionary):** `numeric`  
    - **Meaning:** Bidder latency in milliseconds between the request and the bid response.  
    - **Expectation:** Non-negative integer/numeric values (latency in ms) suitable for arithmetic summaries and modeling.  
    - **Actual Type:** `character`  
    - **Missing:** None

### Exploration

We inspect a small sample and inspect patterns:

```{r response-time-sample}
sample(ad_clean$RESPONSE_TIME, size = 20)
```
...
# Check common leading patterns (first 15 characters)
rt_prefix_summary <- ad_clean %>%
  transmute(prefix15 = str_sub(RESPONSE_TIME, 1, 15)) %>%
  count(prefix15, sort = TRUE)
...
**Observations**
  ...
  - `RESPONSE_TIME` is stored as `character`, even though it represents a numeric value.  
  - Almost all rows begin with the text prefix `"RESPONSE_TIME: "`, with a single row showing a typo `"RESSPONSE_TIME: "`.  
  - The desired numeric content appears to be the digits following the prefix, and any trailing non-digit characters are extraneous.
  - There are no structurally missing values: every row contains a string with this general pattern.

**Decisions:**

  - We extract the numeric portion of `RESPONSE_TIME`:
      - Remove any leading non-digit prefix (including typo variants of `"RESPONSE_TIME"`).  
      - Remove any trailing non-numeric characters.  
  - Convert the remaining digits to a numeric variable `RESPONSE_TIME_clean`.  
  - Verify that parsing does not introduce unexpected `NA`s; if any NA values appear, they will be interpreted as malformed latency strings.

### Cleaning

We implement the prefix/suffix removal and numeric conversion, then check for any resulting `NA`s:

```{r response-time-clean, results='hold'}
ad_clean <- ad_clean %>%
  mutate(
    RESPONSE_TIME_clean = RESPONSE_TIME %>%
      # Remove any leading non-digit characters (prefix, including typos)
      str_replace("^[^0-9]+", "") %>%
      # Remove any trailing non-digit characters
      str_replace("[^0-9]+$", "") %>%
      # Convert to numeric (latency in ms)
      as.numeric()
  )

n_rt_na <- sum(is.na(ad_clean$RESPONSE_TIME_clean))

cat("Rows with NA in RESPONSE_TIME_clean after parsing: ",
    n_rt_na, "\n")
```

Log entry for latency variable:

```{r response-time-log, results='hold'}
log_change(
  step_id = "RT1",
  variable = "RESPONSE_TIME",
  issue = "Stored as character with text prefixes (e.g. 'RESPONSE_TIME: ') and possible trailing non-numeric characters.",
  decision = "Stripped non-numeric prefix/suffix using regular expressions and converted the remaining digits to numeric.",
  rows_affected = nrow(ad_clean)
)
```
...
### Summary
  ...
  - `RESPONSE_TIME` was recorded as a character field with a consistent `"RESPONSE_TIME: "`-style prefix and one typo variant.
  - We extracted only the numeric portion by removing any leading non-digits and trailing non-numeric characters, then converted the result to a numeric latency variable `RESPONSE_TIME_clean`.  
  - This cleaning step was performed without dropping any rows from the dataset.
...
- `BID_WON`  
  ...
  - **Meaning:** Indicator for whether this bid line is the **winning** bid in its auction.  
  - **Expectation:** A logical TRUE/FALSE flag with exactly one `TRUE` per `AUCTION_ID`.  
  - **Actual Type:** `character`  
  - **Missing:** None

### Exploration

We inspect the distinct string values used to encode the outcome and check for case variations.

```{r bid-won-explore, results='hold'}
# Distinct values and counts for BID_WON
bid_won_summary <- ad_clean %>%
  count(BID_WON, sort = TRUE)

bid_won_summary
```

  - We see three values present: `"FALSE"`, `"TRUE"`, `"true"`

We also check that there is still **exactly one** bid won per auction:
```{r bid-won-auction, results='hold'}
auction_win_summary <- ad_clean %>%
  group_by(AUCTION_ID) %>%
  summarise(
    n_rows = n(),
    n_won  = sum(BID_WON %in% c("TRUE", "true"),  na.rm = TRUE),
    n_lost = sum(BID_WON %in% c("FALSE"),         na.rm = TRUE),
    .groups = "drop"
  )
...
We see that converting both `"TRUE"` and `"true"` to `TRUE` results in **9 auctions with multiple winners**.

We see how many rows this affects by inspecting associated `AUCTION_ID`'s:
```{r multiwin-rowcheck}
multiwin_ids <- auctions_multiwin$AUCTION_ID
...
**Observations**
  ...
  - `BID_WON` is stored as a character flag rather than a logical variable.
  - There are three distinct values for `BID_WON`: `"FALSE"`, `"TRUE"`, `"true"`
  - We found there are **9 auctions** and **`r multiwin_rows` associated rows** which now violate the **exactly one winner** per `AUCTION_ID` criteria.

**Decisions**

  - Convert `BID_WON` from a character flag into a proper logical variable `BID_WON_clean`.  
  - Map `"TRUE"` and `"true"` to `TRUE`, and `"FALSE"` to `FALSE`. 
  - Since an auction must have *exactly one* winner and the number of affected rows is very small, we remove all auctions where `n_won != 1`.

### Cleaning

We re-code `BID_WON` from character to logical.

```{r bid-won-clean, results='hold'}
ad_clean <- ad_clean %>%
  mutate(
...
n_bid_won_na <- sum(is.na(ad_clean$BID_WON_clean))

cat("Rows with NA in BID_WON_clean after recoding: ",
    n_bid_won_na, "\n")

# Remove auctions that violate the "exactly one winner" rule
invalid_ids_bw <- auctions_multiwin$AUCTION_ID

n_bw_invalid_rows <- ad_clean %>%
  filter(AUCTION_ID %in% invalid_ids_bw) %>%
  nrow()

ad_clean <- ad_clean %>%
  filter(!AUCTION_ID %in% invalid_ids_bw)

cat("Rows removed due to multi-winner auctions: ",
    n_bw_invalid_rows, "\n")
```

Log entry for outcome variable:

```{r bid-won-log, results='hold'}
log_change(
  step_id = "BW1",
  variable = "BID_WON",
  issue = "Outcome flag stored as character with three string encodings ('TRUE', 'true', 'FALSE').",
  decision = "Standardized to logical BID_WON_clean by mapping TRUE/true to TRUE and FALSE to FALSE.",
  rows_affected = nrow(ad_clean)
)
...
log_change(
  step_id = "BW2",
  variable = "BID_WON",
  issue = "Auctions with multiple winning bids after standardizing TRUE/true.",
  decision = "Removed all rows belonging to the 9 multi-winner auctions.",
  rows_affected = n_bw_invalid_rows
)
```

### Summary

  - `BID_WON` is a key outcome flag but was stored as a character variable with three distinct encodings: `"FALSE"`, `"TRUE"`, and `"true"`.  
  - We created `BID_WON_clean`, a logical TRUE/FALSE variable, by mapping `"TRUE"`/`"true"` to `TRUE` and `"FALSE"` to `FALSE`, reserving `NA` for any unexpected future labels.  
  - Using this unified winner flag, we identified a small set of auctions with more than one winner. These auctions violate the business rule of “exactly one winner per auction,” so all rows belonging to those auctions were removed from the cleaned dataset.
...
We now construct the final cleaned dataset by:

  - Keeping only columns that represent cleaned variables (those ending in `"_clean"`), and  
  - Renaming them back to their original variable names by removing the `"_clean"` suffix.

```{r assemble-ad-clean, results='hold'}
# Identify cleaned columns
all_cols   <- names(ad_clean)
clean_cols <- all_cols[str_detect(all_cols, "_clean$")]

# Keep only cleaned columns and rename by stripping the "_clean" suffix
ad_clean <- ad_clean %>%
...
At this point, `ad_clean` contains only the cleaned variables with their original dictionary names (e.g., `TIMESTAMP`, `PRICE`, `BID_WON`).

---

## Post-Cleaning Inspection

We perform a few final checks:

  - Dimensions of the cleaned dataset,  
  - Per-column missingness,  
  - Verification that no new full-row duplicates were introduced during cleaning.

```{r post-cleaning-inspection, results='hold'}
# Dimensions
n_rows_final <- nrow(ad_clean)
n_cols_final <- ncol(ad_clean)

cat("Final number of rows:    ", n_rows_final, "\n")
cat("Final number of columns: ", n_cols_final, "\n\n")

# Per column Summary
summarize_ad_data(ad_clean)

# Check for full-row duplicates in the final cleaned dataset
n_distinct_final <- ad_clean %>%
  distinct() %>%
  nrow()

n_dups_final <- n_rows_final - n_distinct_final

cat("\nExact duplicate rows in final dataset: ", n_dups_final, "\n")
```

In this notebook run, `n_dups_final` reports whether any full-row duplicates remain after all cleaning steps; since we removed exact duplicates near the beginning and only performed deterministic transformations thereafter, we do **not** expect new duplicates to be introduced.

---

## Save cleaned dataset via `export_ad_data()`  (Optional)

Finally, we export the cleaned dataset to Parquet using the helper in `data_io.R`.  
The output is written to the default directory (`Project/data/processed/`) with a versioned filename.  
If re-run with further changes, bump the `version` string (e.g., `"clean_v2"`) or set `overwrite = TRUE`.

```{r export-cleaned-data, eval=FALSE}
export_path <- export_ad_data(
  df      = ad_clean,
  version = "clean_v1",
  overwrite = FALSE  # set to TRUE only if intentionally replacing an existing file
)

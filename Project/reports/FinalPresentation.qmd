...
Missing information refers to fields recorded as NA or left blank across one or more rows and/or columns. Furthermore, there is a need to distinguish if these missing values are a feature of the dataset or bad data.
...
Duplicate rows contain the exact same information for every single column. Near-duplicate rows contain the exact same information for key columns that pertain to future exploration.

## Examples

Example of Near-Duplicates (excluding geolocational metadata):

- Grouping by AUCTION_ID, PUBLISHER_ID, RESPONSE_TIME, PRICE and SIZE results in duplicate rows.

## Why It Matters

Having duplicate/near-duplicate data:

- Inflates the amount of rows, which can skew results.
- Puts to question the validity of final conclusions once analysis is performed.

## Cleaning Strategy (Fix Summary) 

- Principle: Remove duplicates/acknowledge near-duplicates.
- Implementation: Remove duplicates; identify near-duplicates through grouping on key fields; determine potential impact of near-duplicates.
...
- .../STAT531_Project/Project (root):
    ...
    - reports/ (saved plots)
- Snake case naming convention utilized.
